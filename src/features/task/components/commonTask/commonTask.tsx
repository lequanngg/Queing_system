...
import DeleteArchiveTask from "../deleteArchiveTask/DeleteArchiveTask";
...
export const CommonTask: React.FC = (resultCommon: any) => {
  const list = useSelector((state: RootState) => state.task.tasks);

  const listOther = list.filter((task) => task.type === 0);
  ...
  return (
    <div>
      <Title>
        <h5>
          Common Task ({listSearch.length})<div> </div>
        </h5>
